...
class ImageEditThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            # Resolve resolution
            # Resolve resolution and aspect ratio
            orig_w, orig_h = self.original_size
            ratio = orig_w / orig_h if orig_h > 0 else 1.0

            target_long_edge = 1024  # Default
            ...
            if self.resolution == "4096":
                ...
                if max_dim > 2048:
                    target_long_edge = 4096
                    print(
                        f"[NANO BANANA] Auto-resolution: Detected large image ({max_dim}px) -> Setting 4K"
                    )
                elif max_dim > 1024:
                    target_long_edge = 2048
                    print(
                        f"[NANO BANANA] Auto-resolution: Detected medium image ({max_dim}px) -> Setting 2K"
                    )
                else:
                    target_long_edge = 1024
                    print(
                        f"[NANO BANANA] Auto-resolution: Detected standard image ({max_dim}px) -> Setting 1K"
                    )

            # Calculate new dimensions maintaining aspect ratio
            if orig_w >= orig_h:
                width = target_long_edge
                height = int(target_long_edge / ratio)
            else:
                height = target_long_edge
                width = int(target_long_edge * ratio)

            print(
                f"[NANO BANANA] Target dimensions: {width}x{height} (Ratio: {ratio:.2f})"
            )
        ...
        def load_image():
            try:
                        ...
                        if area.type == "IMAGE_EDITOR":
                            for space in area.spaces:
                                if space.type == "IMAGE_EDITOR":
                                    ...
                                    switched = True
                                    print(
                                        f"[NANO BANANA] Switched window {window.as_pointer()} to edited image"
                                    )
                ...
                if switched:
                    ...
                    print(
                        f"[NANO BANANA] Warning: No Image Editor found to display result"
                    )
