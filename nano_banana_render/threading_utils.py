...
def save_reference_image_temp(scene) -> str:
    ...
    try:
        ...
        if (
            not props
            or not props.use_style_reference
            or not props.style_reference_image
        ):
            return None
            ...
            try:
                ...
                try:
                    ...
                    pixel_array = np.array(pixels).reshape(
                        (height, width, reference_image.channels)
                    )
...
def load_result_image(
    image_data: bytes, image_name: str = "AI_Result", user_prompt: str = ""
) -> None:
    """Load result image into Blender and save to history (thread-safe)"""
    print(
        f"🚀 [GEMINI] load_result_image called with user_prompt: '{user_prompt}', image_name: '{image_name}'"
    )
        ...
        try:
            ...
            try:
                ...
                if user_prompt:  # Only if we need to save to history
                    ...
                    permanent_name = (
                        f"AI_Result_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
                    )
                    ...
                    print(
                        f"[GEMINI] Original image has_data: {permanent_image_for_history.has_data}, size: {permanent_image_for_history.size}"
                    )

                # Method 1: Try to replace existing Render Result completely
                print(
                    f"[GEMINI] Loaded image: {img.name}, size: {img.size}, channels: {img.channels}"
                )
                    ...
                    if area.type == "IMAGE_EDITOR":
                        for space in area.spaces:
                            if space.type == "IMAGE_EDITOR":
                                ...
                                updated_editors += 1
                                print(
                                    f"[GEMINI] Updated Image Editor {updated_editors}"
                                )
                ...
                try:
                    ...
                    try:
                        ...
                        new_window = bpy.context.window_manager.windows[
                            -1
                        ]  # Last created window
                            ...
                            if area.type != "IMAGE_EDITOR":
                                ...
                                for space in area.spaces:
                                    if space.type == "IMAGE_EDITOR":
                                        ...
                                        area.tag_redraw()
                                        print(
                                            "[GEMINI] New window created with render result!"
                                        )
                        ...
                        try:
                                ...
                                if area.type == "EMPTY":
                                    ...
                                    for space in area.spaces:
                                        if space.type == "IMAGE_EDITOR":
                                            ...
                                            area.tag_redraw()
                                            print(
                                                "[GEMINI] Duplicated area set to show render result"
                                            )
                                            break  # Exit spaces loop only
                                    break  # Exit areas loop only
                            ...
                            try:
                                    ...
                                    if area.type in SAFE_AREAS_TO_CONVERT:
                                        ...
                                        for space in area.spaces:
                                            if space.type == "IMAGE_EDITOR":
                                                ...
                                                area.tag_redraw()
                                                print(
                                                    f"[GEMINI] Safely converted {old_type} to Image Editor"
                                                )
                                                area_converted = True
                                                break  # Exit spaces loop only
                                        break  # Exit areas loop only
                                ...
                                if not area_converted:
                                    print(
                                        "[GEMINI] No SAFE areas found, trying existing Image Editor..."
                                    )
                                        ...
                                        if area.type == "IMAGE_EDITOR":
                                            for space in area.spaces:
                                                if space.type == "IMAGE_EDITOR":
                                                    ...
                                                    area.tag_redraw()
                                                    print(
                                                        "[GEMINI] Using existing Image Editor for render result"
                                                    )
                                                    break  # Exit spaces loop only
                                            break  # Exit areas loop only

                                    print(
                                        "💡 [GEMINI] Render result is available as 'Render Result' image in Blender"
                                    )

                            except Exception as e3:
                                print(f"[GEMINI] Safe area conversion failed: {e3}")
                                print(
                                    "[GEMINI] Render result loaded - access manually via Image Editor"
                                )
                ...
                print(
                    "[GEMINI] Finished window/editor setup, now starting history save..."
                )

                # Add to render history
                print(
                    f"[GEMINI] Attempting to save to history, user_prompt: '{user_prompt}'"
                )
                if user_prompt:
                ...
                if user_prompt:
                    try:
                        ...
                        print(
                            f"[GEMINI] Scene has gemini_render: {hasattr(scene, 'gemini_render')}"
                        )

                        if hasattr(scene, "gemini_render"):
                            props = scene.gemini_render
                            print(
                                f"[GEMINI] Current history length: {len(props.render_history)}"
                            )
                            ...
                            if permanent_image_for_history:
                                history_item.image_name = (
                                    permanent_image_for_history.name
                                )
                                print(
                                    f"[GEMINI] Using pre-created permanent image for history: {permanent_image_for_history.name}"
                                )
                                print(
                                    f"[GEMINI] Pre-created image has_data: {permanent_image_for_history.has_data}, size: {permanent_image_for_history.size}"
                                )
                            else:
                                ...
                                try:
                                    ...
                                    print(
                                        f"[GEMINI] Created fallback permanent image: {permanent_name}"
                                    )
                                except Exception as e:
                                    print(
                                        f"[GEMINI] Failed to create fallback permanent copy: {e}"
                                    )
                                    history_item.image_name = render_result.name
                                    print(
                                        f"[GEMINI] Using render result directly as last resort: {render_result.name}"
                                    )

                            print(
                                f"[GEMINI] History item created: {history_item.prompt[:30]}..."
                            )

                            # Save style reference info if used
                            if (
                                props.use_style_reference
                                and props.style_reference_image
                            ):
                                history_item.style_reference_used = True
                                history_item.style_reference_name = (
                                    props.style_reference_image.name
                                )
                                print(
                                    f"[GEMINI] Style reference saved: {props.style_reference_image.name}"
                                )

                                # NO THUMBNAIL CREATION - style thumbnails removed
                                history_item.style_reference_thumbnail = ""
                                print(
                                    f"[GEMINI] Style reference linked (no thumbnail created)"
                                )
                            else:
                                history_item.style_reference_used = False
                                print(
                                    "[GEMINI] No style reference used for this render"
                                )

                            # Add timestamp
                            import datetime

                            history_item.timestamp = datetime.datetime.now().strftime(
                                "%Y-%m-%d %H:%M:%S"
                            )
                            ...
                            print(
                                f"[GEMINI] No thumbnail created - using main AI_Result image directly"
                            )
                            ...
                            while len(props.render_history) > 10:
                                ...
                                if oldest.image_name in bpy.data.images:
                                    ...
                                    print(
                                        f"[GEMINI] Removing old AI_Result image: {oldest.image_name}"
                                    )
                            ...
                            print(
                                f"[GEMINI] Added to history: {user_prompt[:50]}... (Total: {len(props.render_history)})"
                            )
                        ...
                        print(
                            f"[GEMINI] History error traceback: {traceback.format_exc()}"
                        )
...
class APIThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            # Get resolution from scene render settings (for aspect ratio) and resolution tier from props
            props = (
                self.scene.gemini_render
            ...
            print(f"[GEMINI] Using scene render resolution: {width}x{height}")
            ...
            try:
                ...
                print(
                    f"[GEMINI] AI response received, image size: {len(image_data)} bytes"
                )
            finally:
            ...
            print(
                f"[GEMINI] About to call load_result_image with user_prompt: '{self.user_prompt}' (length: {len(self.user_prompt) if self.user_prompt else 0})"
            )
...
class FullRenderThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            render_mode = (
                props.render_mode
                if props and hasattr(props, "render_mode")
                else "DEPTH"
            )
            ...
            if render_mode == "DEPTH":
                ...
                mist_falloff = (
                    props.mist_falloff
                    if props and hasattr(props, "mist_falloff")
                    else "LINEAR"
                )
                ...
                def _do_safe_eevee_render():
                    ...
                    try:
                        ...
                        depth_path = self.depth_renderer.render_regular_eevee(
                            self.scene
                        )
                        render_result = "success"
            ...
            # Get resolution from scene render settings (for aspect ratio)
            render = self.scene.render
            width = render.resolution_x
            height = render.resolution_y
            print(f"[GEMINI] Using scene render resolution: {width}x{height}")
